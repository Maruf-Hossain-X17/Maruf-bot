...
module.exports = {
        config: {
                ...
                role: 2,
                description: {
                        vi: "Quản lý các tệp lệnh của bạn (Chỉ dành cho Maruf)",
                        en: "Manage your command files (Owner Only)"
                },
                category: "owner",
                guide: {
        ...
        onStart: async ({ args, message, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, event, commandName, getLang }) => {
                // 🔒 RESTRICT USE TO MARUF ONLY (UID ending in 904)
                if (String(event.senderID) !== ADMIN_UID) {
                        return message.reply("⛔ | বস, এই কমান্ডটি শুধুমাত্র ডেভেলপার মারুফ (Maruf) ব্যবহার করতে পারবে!");
                }
                ...
                if (
                        ...
                        if (url.match(/(https?:\/\/(?:www\.|(?!www)))/)) {
                                if (global.utils.log.dev) global.utils.log.dev("install", "url", url);
                                ...
                                if (global.utils.log.dev) global.utils.log.dev("install", "code", args.slice(1).join(" "));
        ...
        onReaction: async function ({ Reaction, message, event, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang }) {
                ...
                // 🔒 RESTRICT REACTION INSTALL TO MARUF ONLY
                if (String(event.userID) !== ADMIN_UID || String(author) !== ADMIN_UID)
                        return;
...
function loadScripts(folder, fileName, log, configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang, rawCode) {
        ...
        try {
                ...
                // —————————————— CHECK COMMAND EXIST ——————————————— //
                if (oldCommandName) {
                        if (GoatBot[setMap].get(oldCommandName)?.location && GoatBot[setMap].get(oldCommandName)?.location != pathCommand)
                ...
                // 🛠️ FIX: Check onFirstChat function correctly
