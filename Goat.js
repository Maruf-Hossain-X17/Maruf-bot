...
app.use(express.json()); // <--- **CRITICAL ADDITION**
// const port = Math.floor(Math.random() * 50000) + 50000;  <-- এই লাইনটি সরানো হয়েছে
...
app.post("/api/appstate", (req, res) => {
        // req.body now correctly contains the JSON payload thanks to app.use(express.json());
...
// -----------------------------------------------------------------
// Render-এর জন্য এই অংশটি যুক্ত করুন যাতে ডিপ্লয়মেন্ট সফল হয়
// -----------------------------------------------------------------

const RENDER_PORT = process.env.PORT || 8080; // Render-এর PORT ভ্যারিয়েবল ব্যবহার করুন

// 0.0.0.0 হোস্ট-এ বাইন্ড করা নিশ্চিত করুন
app.listen(RENDER_PORT, '0.0.0.0', () => {
    log.info(`Dummy Web Server running on port ${RENDER_PORT} to satisfy Render's port scan.`);
});
