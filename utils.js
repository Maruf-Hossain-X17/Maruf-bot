/**
 * Goat Bot V2 - utils.js
 * Google credentials are OPTIONAL.
 *
 * Original: NTKhang03
 * Enhanced: error handling, retry, timeout, performance, memory safety
 * Updated: Google Drive can be disabled when credentials are missing
 */
 ...
 * ============================================================
 *
 * The bot no longer exits when Google credentials are missing.
 *
 * Google Drive will only be enabled when all three are present:
 * - clientId
 * - clientSecret
 * - refreshToken
 */
...
if (clientId && clientSecret && refreshToken) {
    try {
        ...
        log.warn?.(
            "GOOGLE",
            `Google Drive disabled: ${err.message}`
        );
    }
} else {
    log.warn?.(
        "GOOGLE",
        "Google Drive disabled: Google credentials are not configured."
    );
}

/*
 * Helper used by Google Drive functions.
 * The bot itself can run normally without Google credentials.
 */
function requireDrive() {
    if (!driveApi) {
        throw new Error(
            "Google Drive is disabled because Google credentials are not configured."
        );
    }
}
...
function convertTime(
    ...
    const month = Math.floor(
        (miliSeconds / 1000 / 60 / 60 / 24 / 30) % 12
    );
    ...
    const dateParts = [
        {
            value: year,
            replace: replaceYears
        },
        {
            value: month,
            replace: replaceMonths
        },
        {
            value: day,
            replace: replaceDays
        ...
        {
            value: minute,
            replace: replaceMinutes
        },
        {
            value: second,
            replace: replaceSeconds
        }
    ];
    ...
    for (let i = 0; i < dateParts.length; i++) {
        ...
        if (value) {
            formattedDate += value + replace;
        } else if (formattedDate !== "") {
            formattedDate += "00" + replace;
...
function enableStderrClearLine(isEnable = true) {
    process.stderr.clearLine = isEnable
...
function formatNumber(number) {
    const regionCode =
        global.GoatBot?.config?.language || "en-US";
    ...
    if (
        ...
        throw new Error(
            "The first argument (number) must be a number"
        );
    }
...
function randomNumber(min, max) {
    ...
    if (
        ...
        throw new Error(
            "The first argument (min) must be a number"
        );
    }
    ...
    if (
        ...
        throw new Error(
            "The second argument (max) must be a number"
        );
    }
...
async function translate(text, lang) {
    if (
        ...
        throw new Error(
            "The first argument (text) must be a string"
        );
    }
    ...
    if (
        ...
        throw new Error(
            "The second argument (lang) must be a string"
        );
    }
    ...
    for (
        ...
        if (
            typeof wordTrans !== "string" ||
            wordTrans.trim().length === 0
        ) {
            output +=
                wordTrans || "";
        ...
        const startSp =
            lengthWhiteSpacesStartLine(
                wordTranslate[i]
            );

        const endSp =
            lengthWhiteSpacesEndLine(
                wordTranslate[i]
            );
...
async function downloadFile(
    ...
    if (
        ...
        throw new Error(
            "The second argument (path) must be a string"
        );
    }

    const tmpPath =
        `${savePath}.tmp-${Date.now()}`;
...
async function findUid(link) {
    if (
        ...
        throw new Error(
            "The first argument (link) must be a string"
        );
    }
    ...
    throw new Error(
        "UID not found. Please try again with a valid link."
    );
}


async function getStreamsFromAttachment(
    attachments = []
) {
    const streams = [];
    ...
    for (
        ...
        try {
            ...
            log.warn?.(
                "ATTACHMENT",
                `Failed to get stream: ${err.message}`
            );
        }
    }
...
async function uploadImgbb(file) {
    ...
    try {
        ...
        if (!authMatch) {
            throw new Error(
                "Unable to get ImgBB auth token"
            );
        }
...
async function uploadZippyshare(stream) {
    try {
        ...
        res.data.data.file.url.download =
            downloadUrl;
...
const drive = {
    ...
    async getFile(
        ...
        const fileName =
            ...
            `${utils.randomString(10)}.${utils.getExtFromMimeType(
                headersResponse[
                    "content-type"
                ]
            )}`;
